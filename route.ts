...
export async function GET(req: NextRequest) {
  ...
  // Note: school.institutional_characteristics.level is select-only in the
  // Scorecard API (not filterable), so filter on predominant degree instead:
  // 2 = predominantly associate (2-year), 3-4 = bachelor's or higher (4-year).
  if (level === "1") params.set("school.degrees_awarded.predominant", "3,4");
  if (level === "2") params.set("school.degrees_awarded.predominant", "2");
