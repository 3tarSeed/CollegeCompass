...
import { MAX_COMPARE, type College } from "@/lib/types";
...
export default function ComparePage() {
  const { ready, profile, colleges, compareIds, toggleCompare, scholarships } = useApp();
  ...
  const sections = useMemo(() => {
    ...
    return [
      ...
      section("Scholarships", [
        {
          label: "Merit scholarships offered",
          best: "none",
          value: () => null,
          render: (c) =>
            c.financialAid?.meritAidAvailable == null
              ? NOT_REPORTED
              : c.financialAid.meritAidAvailable
                ? "Yes"
                : "No",
        },
        {
          ...
          value: () => null,
          render: (c) => {
            const d = c.deadlines.find((x) => x.plan === "scholarship");
            return d ? `${d.label}: ${d.dueDate ? fmtDate(d.dueDate) : "Rolling"}` : NOT_REPORTED;
          },
        },
        {
          label: "Your awarded scholarships (usable here)",
          best: "high",
          value: (c) =>
            scholarships
              .filter((sch) => sch.status === "awarded" && (sch.collegeId === null || sch.collegeId === c.id))
              .reduce((a, sch) => a + (sch.amount ?? 0), 0),
          render: (c) => {
            const total = scholarships
              .filter((sch) => sch.status === "awarded" && (sch.collegeId === null || sch.collegeId === c.id))
              .reduce((a, sch) => a + (sch.amount ?? 0), 0);
            return total > 0 ? `${fmtMoney(total)}/yr` : "None yet";
          },
        },
        {
          label: "Your scholarship applications in progress",
          best: "none",
          value: () => null,
          render: (c) => {
            const n = scholarships.filter(
              (sch) => (sch.status === "planned" || sch.status === "applied") && (sch.collegeId === null || sch.collegeId === c.id),
            ).length;
            return n > 0 ? `${n} planned/applied` : "None";
          },
        },
      ]),
  ...
  if (selected.length === 0) {
    return (
      <EmptyState
        ...
        body={`Add up to ${MAX_COMPARE} colleges to your comparison from Find Colleges or any college page.`}
